...
- Indeed, actuaries may perfer simpler models (like LRMoE) in practice for their interpretability and ease of implementation.
:::
...
- Modelling correlated claims frequency in a single framework
...
## Univariate Frequency and Severity
...
**What expert functions (distributions) should one use?**
...
- In practice, the choice of expert functions depends on the following considerations:
  ...
  - **Domain Knowledge**: Models more powerful when combined with actuaries' discretion.
...
LRMoE with EC experts outperforms both NB GLM and ZINB GLM in terms of fitting the chi-squared
statistic and the log-likelihood.

{{< include tables/EUR-correlated-frequency-fit-table.md >}}
...
{{< include tables/EUR-correlated-frequency-moments-table.md >}}
...
- We compare which losses are identified and how much is saved by eleminating the losses
...
- We assume **claim arrival** follows a non-homogeneous Poisson process, with intensity
  dependent on policyholder covariates.

- Once these components are fitted, we can produce a predicted **distriution** of IBNR claims
  for each policyholder at any valuation date.
:::
...
Implementing the estimation involves some customization based on the selection of 
experts functions, as well as modifications based on the presence of censored and/or truncated data.
...
**customized expert functions** for various modelling problems in and out of the insurance context. 
