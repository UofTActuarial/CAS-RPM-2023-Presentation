...
- Joint work by the [actuarial research group](https://actsci.utstat.utoronto.ca/) at the University of Toronto: \ \ \ \  Spark Tseung, Prof. Tsz Chai (Samson) Fung, Prof. Andrei Badescu, and Prof. X. Sheldon Lin
...
::: incremental
- We first classify policyholders into different latent risk groups with a logistic regression.
...
::: nonincremental
- The MoE framework can catch 3 patterns: regression, dependence, and distributional.
- For example, it offers a much better fit to data compared with GLM, as it can capture the nonlinear relationship between policyholder information and losses.
:::
...
**expert distribution function** \ $f_j(\mathbf{y}_{i}; \mathbf{\varphi}_j)$ \ with parameters $\mathbf{\varphi}_j$.  Note that it **does not** depend on the covariates anymore.
:::
...
- Our package currently supports:
  ...
  - ...and the zero-inflated version of all the above!
