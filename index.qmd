...
**expert function** $f_j(\mathbf{y}_{i}; \mathbf{\phi}_j)$ with parameters $\mathbf{\phi}_j$.
:::
...
::: incremental
- In the expert functions $f_j(\mathbf{y}_{i}; \mathbf{\phi}_j)$, we have assumed some **fixed** parameters
$\mathbf{\phi}_j$ which are **independent** of the covariates $\mathbf{x}_{i}$.

- It is possible to also incorporate the covariates $\mathbf{x}_{i}$ in the parameters of the expert functions such that
$$
    f_j(\mathbf{y}_{i}; \mathbf{\phi}_j(\mathbf{x}_{i})) = f_j(\mathbf{y}_{i}; \mathbf{\phi}_j(\mathbf{x}_{i}, \mathbf{\beta}_j))
$$
